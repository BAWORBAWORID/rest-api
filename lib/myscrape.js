...
axios.get(url).then(({ data }) => {
const $ = cheerio.load(data)
const dlnye = 'https:'+$('#dl-form').attr('action')
  ...
  return new Promise((resolve, reject) => {
   axios.get('https://www.mynimeku.com/?s='+q).then(({ data }) => {
      ...
      $('div.flexbox2 > div.flexbox2-item').each(function(a, b) {
        ...
        let tp = $(b).find('div > div > div.type').text()
        type.push(tp)
      ...
      for (let i = 0; i < title.length; i++) {
        res.push({
          title: title[i],
          type: type.filter(v => v !== 'Manga')[i] ?? 'TV',
          genre: genre[i],
          img: img[i],
          link: link[i],
          synop: sinopsis[i]
...
async function NimeDl(url) {
  return new Promise((resolve, reject) => {
    axios.get(url).then(({ data }) => {
      const $ = cheerio.load(data)
      let dle = $('div.navigasion > a:nth-child(3)').attr('href') ? $('div.navigasion > a:nth-child(3)').attr('href') : $('div.navigasion > a:nth-child(2)').attr('href')
    let bat = $('div.navigasion > a:nth-child(3)').text() ? $('div.navigasion > a:nth-child(3)').text() : $('div.navigasion > a:nth-child(2)').text()
      if (bat === 'Download Batch') {
...
const SearchNabi = (nabi) => {
   return new Promise( async (resolve, reject) => {
       const scraper = JSON.parse(fs.readFileSync(__path +`/data/islamic/kisahNabi/${nabi}.json`))
       console.log(scraper)
       const result = {
         name: scraper.name,
...
async search(text) {
return new Promise((resolve, reject) => {
fetch('https://nhentai.xxx/search?q='+text).then((x) => x.text())
.then((xx) => {
let $ = cheerio.load(xx)
let title = [];
let id = [];
let thumb = [];
let res = [];
$('.container > .gallery').each(function(a, b) {
title.push($(b).find('a > .caption').text())
id.push($(b).find('a').attr('href').replace('/g/', '').replace('/', ''))
thumb.push($(b).find('a > img').attr('src'))
})
...
async getId(id) {
  return new Promise((resolve, reject) => {
    axios.get('https://nhentai.xxx/g/'+id).then(({ data }) => {
      const $ = cheerio.load(data)
      let result = {}
      let im = []
...
result.image = im
      $('#thumbnail-container > div').each(function(a, b) {
        im.push($(b).find('a > img').attr('data-src').replace('t.', '.'))
      })
      resolve(result)
    })
  })
...
async function CodeGenerate(q) {
  return new Promise((resolve, reject) => {
    const date = {
      fields: {
      ENTER_YOUR_REQUEST_HERE: q
      },
      id: 'bb5baafbb9294e7d6edb6abcfef08190',
      stream: true
    }
fetch(`https://smartgenerator.org/wp-json/mwai-ui/v1/forms/submit`, {
      method: 'post',
  ...
  headers: {
    ...
    .then((pp) => {
      let mm = pp.split('data: {')
      let poo = mm.pop()
      let pl = poo.split('"type":"end",')
      let pk = JSON.parse(`{${pl[1].split('data :').pop()}`)
      let kl = JSON.parse(pk.data)
      resolve({
        status: true,
        author: 'azamibot',
        result: kl.reply
...
async function codeFixed(note, code) {
  note = note ? note : 'Fix This Code'
  return new Promise((resolve, reject) => {
axios(`https://smartgenerator.org/wp-json/mwai-ui/v1/forms/submit`, {
      method: 'post',
      data: {
        fields: {
      ADDITIONAL_INSTRUCTIONSNOTES: note,
      PASTE_HERE_YOUR_CODE: code
        },
        id: '6f74f066b0dcf42c60455c1c40cf4193',
        stream: true
      },
      headers: {
        Accept: 'text/event-stream',
  ...
  let sp = data.split('data: {"type":"end",')
  let zx = sp.pop()
  let js = JSON.parse(`{${zx.split('data: ').pop()}`)
  let rese = JSON.parse(js.data)
  resolve(rese)
    })
  })
}
...
const baseUrl = "https://receive-smss.com/"
async function getNumb() {
  return new Promise((resolve, reject) => {
    axios.get(baseUrl).then(({ data }) => {
      const $ = cheerio.load(data)
      const numb = [];
      $('.number-boxes>a').each(function(a, b) {
        numb.push('+'+$(b).attr('href'))
      })
      let regex = /\/sms\/(\d+)\//
      let no = numb.map(text => text.match(regex)[1]);
      resolve(no)
    })
  });
}

async function getMess(no) {
  let numbe = await getNumb()
  if (numbe.includes(no)) {
  return new Promise((resolve, reject) => {
    let form = [];
    let mess = [];
    let at = [];
    let res = [];
    axios.get(baseUrl+'sms/'+no).then(({ data }) => {
      const $ = cheerio.load(data)
      $('.list-view > div').each(function(a, b) {
        form.push($(b).find('.sender > a').text())
        mess.push($(b).find('.msg > span').text())
        at.push($(b).find('.time').text().replace('Time', ''))
      })
  ...
  resolve({
  status: false,
    message: `${no} Tidak Di Temukan`
  })
}
}

async function AllDl(url) {
  const getToken = await axios.get('https://aiovd.com')
  const $ = cheerio.load(getToken.data)
  const token = $('#token').attr('value')
  const body = new URLSearchParams({ url, token })
  return new Promise((resolve, reject) => {
    axios(`https://aiovd.com/wp-json/aio-dl/video-data/`, {
      method: 'POST',
      data: body,
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
...
async function TwitterDl(url) {
  return new Promise((resolve, reject) => {
    axios(`https://twtube.app/en/download?url=${url}`, {
      method: 'GET',
      data: {
        url
      }
      ...
      const $ = cheerio.load(data)
      const link = [];
      $('.row > .col-md-4').each(function(a, b) {
        link.push($(b).find('div > .square-box-btn > a').attr('href'))
      })
      resolve(link)
    })
  });
...
async function ktpmaker(nik, nama, ttl, gender, alamat, rtrw, lurah, camat, prov, kota, agama, status, kerja, wni, until, ll) {
const { data } = await axios.get(`https://tools.revesery.com/ktp/revesery.php?nik=${nik}&name=${nama}&ttl=${ttl}&jk=${gender}&jl=${alamat}&rtrw=${rtrw}&lurah=${lurah}&camat=${camat}&prov=${prov}&kabu=${kota}&agama=${agama}&nikah=${status}&kerja=${kerja}&warga=${wni}&until=${until}&img=${pp}`)
const $ = cheerio.load(data)
const buffe = $('div > img').attr('src')
return buffe
}

module.exports = { onGoing, BatchAnimeS, quotesanime, pornovid, hentaivid, WibuDl, xnxxs, xnxxdl, tikhd, WibuSearch, BokepRandom, OtakuS, tikcd, ighd, pin, wallpaper, ytplay, ytvid, ytmplay, ytmpvid, isgd, fbvid, fbimg, ai, kfdl, tts, txtoimg, TTStalk, igs, botTT, allindl, unsplash, ytplay2, WallFlare, mynime, NimeDl, xvideos, XDL, fetchText, axiosText, SearchNabi, nHentai, tebakGambar, CodeGenerate, codeFixed, processing, getNumb, getMess, AllDl, TwitterDl, ktpmaker }